...
import laborai.demo.SunkvežimiuGamyba;
import laborai.demo.Sunkvežimis;
...
public class Lab2WindowFX extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static SortedSetADTx<Sunkvežimis> autoSet;
    ...
    private void treeGeneration(String filePath) throws MyException {
        ...
        Sunkvežimis[] sunkvežimisArray;
        // Jei failas nenurodytas - generuojama
        if (filePath == null) {
            sunkvežimisArray = SunkvežimiuGamyba.generuotiIrIsmaisyti(sizeOfGenSet, sizeOfInitialSubSet, coef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            autoSet.load(filePath);
            sunkvežimisArray = new Sunkvežimis[autoSet.size()];
            int i = 0;
            for (Object o : autoSet.toArray()) {
                sunkvežimisArray[i++] = (Sunkvežimis) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(sunkvežimisArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        autoSet.clear();
        for (Sunkvežimis a : sunkvežimisArray) 
        {
            autoSet.add(a);
        }
        // Išvedami rezultatai
    ...
    private void treeAdd() throws MyException {
        KsFX.setFormatStartOfLine(true);
        Sunkvežimis sunkvežimis = SunkvežimiuGamyba.gautiIsBazes();
        autoSet.add(sunkvežimis);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));   
        KsFX.oun(taOutput, sunkvežimis, MESSAGES.getString("msg7"));
        KsFX.oun(taOutput, autoSet.toVisualizedString(tfDelimiter.getText()));
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (autoSet.isEmpty()) {
            ...
            Sunkvežimis sunkvežimis = (Sunkvežimis) autoSet.toArray()[nr];
            autoSet.remove(sunkvežimis);
            KsFX.oun(taOutput, sunkvežimis, MESSAGES.getString("msg6"));
            KsFX.oun(taOutput, autoSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws MyException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                autoSet = new BstSetKTUx(new Sunkvežimis());
                break;
            case 1:
                autoSet = new AvlSetKTUx(new Sunkvežimis());
                break;
            default:
                disableButtons(true);
    ...
    private void treeSubSet()
    {
        ...
        } else
        {
            ...
            if(nr == 0)
                nr++;
            int nr2 = new Random().nextInt(autoSet.size() - (autoSet.size() - nr));
            Sunkvežimis sunkvežimis = (Sunkvežimis) autoSet.toArray()[nr];
            Sunkvežimis sunkvežimis2 = (Sunkvežimis) autoSet.toArray()[nr2];
            KsFX.oun(taOutput, " subSet nuo sito (" + sunkvežimis2 + ") elemento iki (" + sunkvežimis + ")");
            KsFX.oun(taOutput, autoSet.subSet(sunkvežimis2, sunkvežimis).toVisualizedString(tfDelimiter.getText()));
        }
        KsFX.setFormatStartOfLine(false);
    }
    ...
    private void treeTailSet()
    {
        ...
        } else
        {
            int nr = new Random().nextInt(autoSet.size());
            Sunkvežimis sunkvežimis = (Sunkvežimis) autoSet.toArray()[nr];
            KsFX.oun(taOutput, sunkvežimis, " tailSet iki sito elemento");
            KsFX.oun(taOutput, autoSet.tailSet(sunkvežimis).toVisualizedString(tfDelimiter.getText()));
        }
        KsFX.setFormatStartOfLine(false);
    }
}
